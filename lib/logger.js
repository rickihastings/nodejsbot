...
logger.writeLog = function(chan, message)
{
	var date = new Date(),
		...
		filename = 'logging/' + chan.substr(1) + '.' + day + '-' + (date.getMonth() + 1) + '-' + date.getFullYear(),
		details = { flags: 'a',
					encoding: null,
					mode: 0666 },
...
logger.setData = function(obj)
{
	...
	/*
	 * http.get(/\/(.*)?\.(css|js|txt)/i)
	 * route to the css/js/txt files
	 */
	http.get(/\/(.*)?\.(css|js|txt)/i, function(req, res)
	{
	...
	irc.on(/^:[^!]+![^@]+@[^ ]+ (PRIVMSG|NOTICE) #[^ ]+ :(.*)$/, function(info)
	{
		...
		else
		{
			...
			var data = fs.readFileSync('data/seen.json', 'utf-8'),
				seenJson = JSON.parse(data),
			...
			fs.writeFileSync('data/seen.json', JSON.stringify(seenJson));
	...
	irc.on(/^:[^!]+![^@]+@[^ ]+ NICK :[^ ]+/, function(info)
	{
        ...
        {
        	if (nick in helper.oc(logger.chans[chan]))
				logger.writeLog(chan, '* ' + nick + ' is now known as ' + newNick);
	
			delete logger.chans[channel][nick];
			logger.chans[channel].push(newNick);
        }
	});
};
...
logger.indexParser = function(file, req, res)
{
	fs.readFile(file, function(err, data)
	{
    	...
    	var files = fs.readdirSync('logging'),
    		logChans = {};
...
logger.logParser = function(file, req, res)
{
	fs.readFile(file, function(err, data)
	{
		...
		if (err || !path.existsSync('logging/' + fileName))
		{
			res.header('Content-Type', 'text/plain');
			res.end('Cannot GET ' + req.originalUrl);
			return;
    	...
    	var	log = fs.readFileSync('logging/' + fileName, 'utf8');
