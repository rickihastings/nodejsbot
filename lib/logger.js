...
{
	helper = require('lib/main').helper;
	irc = require('lib/main').irc;
	...
	logger.writeLog = function(chan, message)
	{
		var date = new Date(),
			...
			filename = 'logs/' + chan.substr(1) + '.' + day + '-' + date.getMonth() + '-' + date.getFullYear(),
			details = { flags: 'a',
						encoding: null,
						mode: 0666 },
		...
		log.write('[' + hours + ':' + minutes + '] ' + message + '\n');
		log.end();
	}
	...
	irc.on(/^:[^!]+![^@]+@[^ ]+ (PRIVMSG|NOTICE) #[^ ]+ :(.*)$/, function(info)
	{
		...
		logger.writeLog(channel, '<' + nick + '> ' + message);
	});
	...
	irc.on(/^:[^!]+![^@]+@[^ ]+ QUIT :(.*)$/, function(info)
	{
		...
		logger.writeLog(channel, nick + ' (' + splitInfo[0].split('!')[1] + ') has quit (' + message + ')');
	});
...
logger.logParser = function(file, req, res)
{
	fs.readFile(file, function(err, data)
	{
		var fileName = req.params.chan + '.' + req.params.day + '-' + req.params.month,
    		log = fs.readFileSync('logs/' + fileName, 'utf8');
