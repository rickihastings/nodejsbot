...
var net = require('net'),
	...
	config = require('lib/config.json').config,
	modules = {};
...
for (var module in config.modules)
{
	...
	modules[file] = require('lib/modules/' + file);
 ...
 */
fs.watchFile('lib/config.json', function(curr, prev)
{
	delete require.cache[__dirname + '/config.json'];
	var newModule = require('lib/config.json');
	config = newModule.config;
	server.debug('main', 'fs.watchFile(): new config file found, reloading: \'config.json\'');
});
